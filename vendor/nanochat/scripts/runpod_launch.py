...
# ── Configuration ─────────────────────────────────────────────────────────────
# IMPORTANT: update REPO_URL to your fork before using train/test.
# See "Fork your nanochat" in the docstring above.
REPO_URL = "https://github.com/YOUR_USERNAME/nanochat"  # ← CHANGE THIS
BRANCH = "main"
...
def get_github_token() -> str:
    if "YOUR_USERNAME" in REPO_URL:
        print("ERROR: REPO_URL still contains 'YOUR_USERNAME'.")
        print("  Update REPO_URL at the top of this script to your nanochat fork.")
        print("  See the 'Fork your nanochat' section in the docstring.")
        sys.exit(1)
...
def _base_startup() -> list[str]:
    ...
    return [
        ...
        # Clone the nanochat fork (with GR-KAN changes).
        # This will fail if REPO_URL still points to karpathy/nanochat (no GR-KAN)
        # or if the fork hasn't been pushed to GitHub yet.
        "rm -rf ~/nanochat",
        f"git clone --branch {BRANCH} https://{tok}@{REPO_URL.replace('https://', '')} ~/nanochat",
        "cd ~/nanochat",

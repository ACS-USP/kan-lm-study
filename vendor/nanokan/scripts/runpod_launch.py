...
SAFE_JOB_COMPONENT_RE = re.compile(r"[^A-Za-z0-9_.-]+")


def safe_job_component(value: str) -> str:
    """Return a filesystem-safe job-id component for equality validation."""
    safe = SAFE_JOB_COMPONENT_RE.sub("-", value.strip())
    while ".." in safe:
        safe = safe.replace("..", ".")
...
GPU_PREFERENCE = [
    ...
    "NVIDIA H100 PCIe",
    "NVIDIA H100 NVL",                # 94 GB — fastest, Flash Attention 3
    # A100s removed — no Flash Attention 3 (Ampere SM 80, FA2 only)
    # "NVIDIA A100-SXM4-80GB",        # 80 GB — removed: no FA3
    # "NVIDIA A100 80GB PCIe",        # 80 GB — removed: no FA3
...
MACHINE_BLACKLIST: set[str] = {
    # Machines known to drop SSH connections mid-flight.
    # Populated as issues are observed.
}
...
def _try_launch_pod(
    ...
    cmd = " && ".join(startup)
    encoded_startup = base64.b64encode(cmd.encode()).decode()
    startup_log = startup_log_path or "/tmp/runpod_startup.log"
    startup_log_dir = str(Path(startup_log).parent)
    docker_args = (
        "bash -lc '"
        f"mkdir -p {startup_log_dir}"
        " && "
        f"echo {encoded_startup} | base64 -d > /tmp/runpod_startup.sh"
        f" && bash /tmp/runpod_startup.sh > {startup_log} 2>&1"
        f"; cat {startup_log}"
        "; /usr/sbin/sshd -D >/tmp/sshd-recovery.log 2>&1 & "
        "sleep 7200"
        "'"
    )
...
def _base_startup(
    ...
    *,
    public_clone: bool = False,
    apply_local_hotfix: bool = True,
    ...
    """
    tok = "GITHUB_TOKEN_REDACTED" if redact_secrets else ("" if public_clone else get_github_token())
    ...
    clone_url = REPO_URL if public_clone else f"https://{tok}@{REPO_URL.replace('https://', '')}"
    ...
    steps = [
        ...
        "rm -rf ~/nanokan",
        f"git clone --branch {BRANCH} {clone_url} ~/nanokan",
        "cd ~/nanokan",
        *( [f"git checkout {repo_ref}"] if repo_ref else [] ),
        *( _local_source_hotfix() if apply_local_hotfix else [] ),
    ...
    if disable_wandb:
        steps.append("export WANDB_MODE=disabled")
    elif wandb_key:
...
def _resume_seed_startup(
    nanochat_base: str,
    source_job_id: str,
    resume_from_step: int,
    model_tag: str,
    source_model_tag: str | None = None,
) -> list[str]:
    """Generate startup commands to seed a new job directory from a source job.

    Creates the target job structure on the network volume, symlinks the
    source job's ClimbMix data and tokenizer into the target, copies the
    checkpoint triplet for the given step, and creates the ``setup_done``
    sentinel so the downstream dataset/tokenizer setup block is skipped.

    Fails fast if any required source file or directory is missing.

    Designed to run BEFORE :func:`_dataset_and_tokenizer_startup`, which
    will see ``setup_done`` already present and skip its work.
    """
    volume_jobs = "/runpod-volume/nanochat/jobs"
    source_base = f"{volume_jobs}/{source_job_id}"
    source_tag = source_model_tag or model_tag
    step_padded = f"{resume_from_step:06d}"
    seed = (
        # Announce what we are doing (visible in the pod console log).
        f"echo 'Seeding target from source job {source_job_id} (resume step {resume_from_step})'"
        # Ensure target job directories exist on the volume.
        f" && mkdir -p {nanochat_base}/base_checkpoints/{model_tag}"
        # Symlink ClimbMix shards — read-only reference data.
        f" && if [ -d {source_base}/base_data_climbmix ]; then"
        f" ln -sfn {source_base}/base_data_climbmix {nanochat_base}/base_data_climbmix"
        f" && echo '  Linked base_data_climbmix from source';"
        f" else"
        f" echo 'ERROR: Source data directory {source_base}/base_data_climbmix/ not found' >&2; exit 1;"
        f" fi"
        # Symlink tokenizer — read-only reference data.
        f" && if [ -d {source_base}/tokenizer ]; then"
        f" ln -sfn {source_base}/tokenizer {nanochat_base}/tokenizer"
        f" && echo '  Linked tokenizer from source';"
        f" else"
        f" echo 'ERROR: Source tokenizer directory {source_base}/tokenizer/ not found' >&2; exit 1;"
        f" fi"
        # Copy the checkpoint triplet for the resume step.
        # Copied (not symlinked) so the target can safely extend training
        # without modifying the source job's checkpoints.
        f" && for f in model_{step_padded}.pt meta_{step_padded}.json optim_{step_padded}_rank0.pt; do"
        f" if [ -f {source_base}/base_checkpoints/{source_tag}/\"$f\" ]; then"
        f" cp {source_base}/base_checkpoints/{source_tag}/\"$f\" {nanochat_base}/base_checkpoints/{model_tag}/\"$f\""
        f" || {{ echo \"ERROR: Copy failed for $f\" >&2; exit 1; }}"
        f" && echo \"  Copied $f\";"
        f" else"
        f" echo \"ERROR: Missing source checkpoint {source_base}/base_checkpoints/{source_tag}/$f\" >&2; exit 1;"
        f" fi;"
        f" done"
        # Signal that data/tokenizer/checkpoint setup is complete.
        f" && touch {nanochat_base}/setup_done"
        f" && echo 'Resume seeding complete'"
    )
    return [seed]
...
def _make_train_startup(
    ...
    resume_source_model_tag: str | None = None,
    ...
    cmds = _base_startup(
        ...
        apply_local_hotfix=apply_local_hotfix,
    ...
    # Resume source seeding: populate target job dir from an existing volume job.
    # This must run BEFORE _dataset_and_tokenizer_startup so that setup_done exists
    # before the if-guard in the dataset block.
    if resume_source_job_id:
        model_tag = model_tag_override or f"d{depth}-{ffn_type}"
        cmds += _resume_seed_startup(
            nanochat_base=nanochat_base,
            source_job_id=resume_source_job_id,
            resume_from_step=resume_from_step or 0,
            model_tag=model_tag,
            source_model_tag=resume_source_model_tag,
        )
    ...
    if resume_from_step is not None:
        resume_bash = (
            f'RESUME_FLAG="--resume-from-step {resume_from_step}"\n'
            f'echo "Resuming from step {resume_from_step} (explicit)"'
        )
    else:
        resume_bash = (
            f"CKPT_DIR={ckpt_dir}\n"
    ...
    stabilizer_flags = "".join(
        f" {flag}={value}"
        for flag, value in stabilizer_values
    ...
    cmds.append(f"echo Train command flags: --depth={depth} --ffn-type={ffn_type} --model-tag={model_tag}{grkan_flags} --fail-fast --debug-finite-steps={debug_finite_steps} --debug-artifact-dir={debug_artifact_dir}{stabilizer_flags}")
    ...
    cmds += [
        ...
        f"ls {nanochat_base}/base_checkpoints/{model_tag}/ > {nanochat_base}/ARTIFACTS.txt",
...
def cmd_train(args):
    ...
    # Validate resume-source-job-id constraints before any launch.
    resume_source_job_id = getattr(args, "resume_source_job_id", None)
    resume_from_step = getattr(args, "resume_from_step", None)
    if resume_source_job_id:
        if not args.volume_id:
            print("ERROR: --resume-source-job-id requires --volume-id; the source job is on a network volume")
            sys.exit(2)
        if not resume_from_step:
            print("ERROR: --resume-source-job-id requires explicit --resume-from-step")
            sys.exit(2)
        safe_component = safe_job_component(resume_source_job_id)
        if safe_component != resume_source_job_id:
            print(f"ERROR: --resume-source-job-id must be a safe job id component, got {resume_source_job_id!r}")
            sys.exit(2)
        target_job_id = args.job_id or make_job_id(model_tag)
        if resume_source_job_id == target_job_id:
            print(f"ERROR: --resume-source-job-id ({resume_source_job_id}) must differ from target --job-id ({target_job_id})")
            sys.exit(2)
        resume_source_model_tag = getattr(args, "resume_source_model_tag", None)
        if resume_source_model_tag and safe_job_component(resume_source_model_tag) != resume_source_model_tag:
            print(f"ERROR: --resume-source-model-tag must be a safe model tag component, got {resume_source_model_tag!r}")
            sys.exit(2)
    ...
    startup = _make_train_startup(
        ...
        max_loss=getattr(args, "max_loss", None),
        embedding_lr=getattr(args, "embedding_lr", None),
        unembedding_lr=getattr(args, "unembedding_lr", None),
        matrix_lr=getattr(args, "matrix_lr", None),
        scalar_lr=getattr(args, "scalar_lr", None),
        weight_decay=getattr(args, "weight_decay", None),
        final_lr_frac=getattr(args, "final_lr_frac", None),
        total_batch_size=getattr(args, "total_batch_size", None),
        public_clone=getattr(args, "public_clone", False),
        apply_local_hotfix=not getattr(args, "no_local_hotfix", False),
    ...
    stabilizer_summary = {k: v for k, v in stabilizer_summary.items() if v is not None}
    if stabilizer_summary:
        print(f"  Stabilizers: {stabilizer_summary}")
    ...
    gpu, pod = find_and_launch_pod(
        ...
        disk_gb=60,
        secure=secure,
        startup_log_path=f"{nanochat_base}/startup.log" if args.volume_id else None,
    )
    pod_id = pod["id"]
...
def _incremental_checkpoint_sync(
    ssh_ip: str, ssh_port: int, dest: Path, quiet: bool = False
) -> list[str]:
    """Rsync new/updated training artifacts from a running pod.

    Only transfers files that are new or changed — safe to call every poll interval.
    Returns list of new checkpoint filenames transferred.
    """
    ssh_opt = _ssh_rsync_opt(ssh_port)
    remote = f"root@{ssh_ip}"
    flags = ["-a", "--partial", "--ignore-existing"] if not quiet else ["-aq", "--partial", "--ignore-existing"]

    # Base checkpoints: model, optimizer, metadata — only new ones.
    ...
    r = subprocess.run(
        ["rsync"] + flags + [
            ...
            "--exclude=*",
            "-e", ssh_opt,
            f"{remote}:~/nanochat_results/base_checkpoints/",
            str(dest) + "/",
        ],
        capture_output=True, text=True,
    )
    new_ckpts = [ln.strip() for ln in r.stdout.splitlines() if ln.strip().endswith(".pt")]

    # Training log — always (appended to, delta-transfer handles efficiently).
    subprocess.run(
        ["rsync"] + (["-a", "--partial"] if not quiet else ["-aq", "--partial"]),
        "-e", ssh_opt,
        f"{remote}:~/nanochat_results/*_train.log", str(dest) + "/",
        capture_output=True, text=True,
    )

    # Run manifest (tiny, always grab).
    subprocess.run(
        ["rsync"] + (["-a", "--partial"] if not quiet else ["-aq", "--partial"]),
        "-e", ssh_opt,
...
def cmd_watch(args):
    ...
    while True:
        ...
        while time.monotonic() < deadline:
            new_ckpts = _incremental_checkpoint_sync(ssh_ip, ssh_port, dest, quiet=True)
            if new_ckpts:
...
def build_parser() -> argparse.ArgumentParser:
    ...
    p_train.add_argument("--repo-ref", default=None,
                         help="nanokan commit/ref to checkout on the pod (default: local HEAD)")
    p_train.add_argument("--public-clone", action="store_true",
                         help="clone REPO_URL without embedding GITHUB_TOKEN in the startup script")
    p_train.add_argument("--no-local-hotfix", action="store_true",
                         help="do not embed or apply local uncommitted source diffs in the pod startup script")
    p_train.add_argument("--disable-wandb", action="store_true",
                         help="set WANDB_MODE=disabled instead of embedding WANDB_API_KEY")
    ...
    p_train.add_argument("--resume-from-step", dest="resume_from_step", type=int, default=None,
                         help="explicit resume step (overrides auto-resume detection)")
    p_train.add_argument("--resume-source-job-id", dest="resume_source_job_id", default=None,
                         help="source job id on the volume to seed checkpoint triplet, data, and tokenizer from (requires --volume-id and --resume-from-step)")
    p_train.add_argument("--resume-source-model-tag", dest="resume_source_model_tag", default=None,
                         help="model tag inside the source job checkpoint directory (default: target --model-tag)")
    p_train.add_argument("--max-grad-norm", dest="max_grad_norm", type=float, default=None,
                         help="pass through to base_train; clips non-Muon gradients when >0")
    p_train.add_argument("--max-loss", dest="max_loss", type=float, default=None,
                         help="pass through to base_train; fail fast when raw loss exceeds this value")
    p_train.add_argument("--embedding-lr", dest="embedding_lr", type=float, default=None,
                         help="override base_train embedding learning rate")
    p_train.add_argument("--unembedding-lr", dest="unembedding_lr", type=float, default=None,
                         help="override base_train unembedding learning rate")
    p_train.add_argument("--matrix-lr", dest="matrix_lr", type=float, default=None,
                         help="override base_train Muon matrix learning rate")
    p_train.add_argument("--scalar-lr", dest="scalar_lr", type=float, default=None,
                         help="override base_train scalar learning rate")
    p_train.add_argument("--weight-decay", dest="weight_decay", type=float, default=None,
                         help="override base_train weight decay")
    p_train.add_argument("--final-lr-frac", dest="final_lr_frac", type=float, default=None,
                         help="override base_train final LR fraction")
    p_train.add_argument("--total-batch-size", dest="total_batch_size", type=int, default=None,
                         help="override base_train total batch size in tokens")

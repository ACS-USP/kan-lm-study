...
# Bounded drain for traceback output.
# After detecting a traceback header line, drain up to this many additional lines
# within this many seconds so the exception message is captured in the log.
_TRACEBACK_DRAIN_MAX_LINES: int = 120
_TRACEBACK_DRAIN_TIMEOUT_SECONDS: float = 8.0


def _drain_traceback(
    proc: subprocess.Popen[object],
    log_file: typing.TextIO,
    *,
    max_lines: int = _TRACEBACK_DRAIN_MAX_LINES,
    timeout_seconds: float = _TRACEBACK_DRAIN_TIMEOUT_SECONDS,
) -> None:
    """Drain remaining output after a traceback header line.

    Reads output until the child exits, the line limit is reached, or the
    timeout expires.  This ensures the exception message and stack frames
    appear in the log before the watchdog terminates the child.
    """
    deadline = time.monotonic() + timeout_seconds
    lines_read = 0
    ...
    while lines_read < max_lines:
        ...
        if not r:
            break  # no more data within the grace period
        try:
        ...
        while True:
            ...
            if line:
                ...
                if event and event.should_stop:
                    failure_event = event
                    if event.kind == "traceback":
                        _drain_traceback(proc, log_file)
                    break

...
def _preflight(
    ...
    *,
    ...
    if (args.resume_from_step is not None) != (args.resume_source_job_id is not None):
        raise WorkflowError("--resume-from-step and --resume-source-job-id must be set together")
...
def _launcher_train_args(
    *,
    ...
    if phase.name == "full" and args.resume_from_step is not None and args.resume_source_job_id is not None:
        cmd += ["--resume-from-step", str(args.resume_from_step), "--resume-source-job-id", args.resume_source_job_id]
...
def build_parser() -> argparse.ArgumentParser:
    ...
    common.add_argument("--resume-from-step", type=int, default=None, help="Global step to resume training from")
    common.add_argument("--resume-source-job-id", default=None, help="Source job ID from which to resume training")

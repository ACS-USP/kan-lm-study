...
def test_train_startup_can_avoid_secrets_and_local_hotfix(monkeypatch):
    monkeypatch.setenv("GITHUB_TOKEN", "ghp_test")
    monkeypatch.setenv("WANDB_API_KEY", "wandb_test")
    monkeypatch.setattr(runpod_launch, "_read_ssh_pubkey", lambda: "ssh-ed25519 AAAATEST")
    monkeypatch.setattr(runpod_launch, "_local_source_hotfix", lambda: ["echo LOCAL_PATCH"])

    startup = runpod_launch._make_train_startup(
        depth=12,
        ffn_type="grkan",
        nanochat_base="/runpod-volume/nanochat/jobs/job123",
        run_name="d12-grkan-g4",
        model_tag_override="d12-grkan-g4",
        grkan_groups=4,
        smoke=False,
        save_every_override=500,
        num_iterations=5040,
        ...
        public_clone=True,
        apply_local_hotfix=False,
        disable_wandb=True,
    )
    script = "\n".join(startup)
    assert "https://github.com/ACS-USP/nanokan" in script
    assert "ghp_test" not in script
    assert "GITHUB_TOKEN_REDACTED" not in script
    assert "LOCAL_PATCH" not in script
    assert "WANDB_MODE=disabled" in script
    assert "wandb_test" not in script


def test_train_startup_supports_stabilized_resume_with_distinct_target_tag(monkeypatch):
    monkeypatch.setenv("GITHUB_TOKEN", "ghp_test")
    monkeypatch.setattr(runpod_launch, "_read_ssh_pubkey", lambda: "ssh-ed25519 AAAATEST")
    monkeypatch.setattr(runpod_launch, "_local_source_hotfix", lambda: [])

    target_tag = "d12-grkan-g4-stab-lr05-clip1"
    startup = runpod_launch._make_train_startup(
        depth=12,
        ffn_type="grkan",
        nanochat_base="/runpod-volume/nanochat/jobs/job123",
        run_name=target_tag,
        model_tag_override=target_tag,
        grkan_groups=4,
        smoke=False,
        save_every_override=100,
        num_iterations=3000,
        repo_ref="abcdef123456",
        rational_kat_cu_ref="41a20b5",
        job_id="job123",
        max_runtime_minutes=60,
        resume_from_step=2520,
        resume_source_job_id="b-g4-20260610T193454Z-full-d12-grkan-g4",
        resume_source_model_tag="d12-grkan-g4",
        max_grad_norm=1.0,
        embedding_lr=0.15,
    ...
    script = "\n".join(startup)
    assert "base_checkpoints/d12-grkan-g4/\"$f\"" in script
    assert f"base_checkpoints/{target_tag}/\"$f\"" in script
    resume_script = _decoded_startup_payload(script, "/tmp/resume.sh")
    assert "--resume-from-step 2520" in resume_script
    train_script = _decoded_startup_payload(script, f"/tmp/train_{target_tag}.sh")
    assert f"--model-tag={target_tag}" in train_script
    assert "$RESUME_FLAG" in train_script
    assert "--save-every=100" in train_script
    assert "--num-iterations=3000" in train_script
    assert "--max-grad-norm=1.0" in train_script
    assert "--embedding-lr=0.15" in train_script
    assert "--unembedding-lr=0.004" in train_script
    assert "--matrix-lr=0.01" in train_script
    assert "--scalar-lr=0.25" in train_script
    assert "--final-lr-frac=0.025" in train_script


def test_launch_wraps_startup_to_hide_shell_dollars_from_graphql(monkeypatch):
    import time
    ...
    class FakeRunpod:
        def create_pod(self, **kwargs):
            ...
            return {"id": "pod_123", "costPerHr": 1.0}

        def get_pod(self, _pod_id):
            return {"machineId": "machine_ok"}

    monkeypatch.setattr(runpod_launch, "runpod", FakeRunpod())
    monkeypatch.setattr(time, "sleep", lambda _seconds: None)

    runpod_launch._try_launch_pod(
        name="test",
        gpu_type_id="NVIDIA H100 80GB HBM3",
        startup=["echo $HOME", "echo $RUNPOD_POD_ID"],
        volume_id=None,
        disk_gb=20,
    )

    docker_args = captured["docker_args"]
    assert "$HOME" not in docker_args
    assert "$RUNPOD_POD_ID" not in docker_args
    assert '"' not in docker_args
    assert "base64 -d > /tmp/runpod_startup.sh" in docker_args
    encoded = re.search(r"echo ([A-Za-z0-9+/=]+) \| base64 -d", docker_args).group(1)
    decoded = base64.b64decode(encoded).decode()
    assert decoded == "echo $HOME && echo $RUNPOD_POD_ID"
...
def test_train_resume_source_job_dry_run_validates_safely(monkeypatch, tmp_path):
    monkeypatch.setenv("GITHUB_TOKEN", "ghp_test")
    monkeypatch.setattr(runpod_launch, "get_api_key", lambda: "rp_test")
    monkeypatch.setattr(runpod_launch, "_read_ssh_pubkey", lambda: "ssh-ed25519 AAAATEST")
    monkeypatch.setattr(runpod_launch, "_local_source_hotfix", lambda: [])
    ...
    args = runpod_launch.build_parser().parse_args(
        [
            ...
            "--resume-from-step",
            "2520",
            "--resume-source-job-id",
            "b-g4-20260610T193454Z-full-d12-grkan-g4",
            "--job-id",
            "b-g4-ext-20260615T210000Z-d12-grkan-g4",
            "--manifest-dir",
            str(tmp_path),
            "--dry-run",
        ]
...
def test_watchdog_drains_traceback_to_log(tmp_path):
    """A multi-line traceback must include the exception message in the log tail."""
    log_path = tmp_path / "train.log"
    artifact_dir = tmp_path / "artifacts"
    cmd = [sys.executable, "-c", "1/0"]
    rc = train_watchdog.run_watchdog(
        train_cmd=cmd,
        log_path=log_path,
        artifact_dir=artifact_dir,
    ...
    assert rc == 42, f"expected failure exit code 42, got {rc}"
    failure_json = artifact_dir / "failure" / "d12-grkan-g8-corrected" / "failure.json"
    failed_sentinel = artifact_dir / "FAILED_d12-grkan-g8-corrected"
    assert failure_json.exists()
    assert failed_sentinel.exists()
    ...
    # The log tail must contain the actual exception, not just the header line.
    log_tail_path = artifact_dir / "failure" / "d12-grkan-g8-corrected" / "last_log_tail.txt"
    assert log_tail_path.exists()
    log_content = log_tail_path.read_text()
    assert "ZeroDivisionError" in log_content, (
        f"expected exception message in log tail, got:\n{log_content}"
    )
